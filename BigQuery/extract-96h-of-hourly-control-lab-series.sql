/*
Extract 96 hours of lab time series of control group icustays before control_onset_time.
---------------------------------------------------------------------------------------------------------------------
- based on https://github.com/BorgwardtLab/mgp-tcn/blob/master/src/query/extract-55h-of-hourly-control-lab-series_ex1c.sql
---------------------------------------------------------------------------------------------------------------------
*/
...
CREATE OR REPLACE TABLE `bdhfa2021.project.control_96h_hourly_labs` AS
SELECT
  pvt.icustay_id
  , pvt.subject_id 
  , pvt.chart_time 
 ...
 --     WHEN itemid = 50889 and valuenum >  10000 THEN null -- mg/dl or mg/L! C-Reactive Protein  Blood Chemistry
  ...
  from `bdhfa2021.project.matched_controls_hourly` ch -- was icustays ie (changed it below as well)
    ...
    LEFT JOIN `physionet-data.mimiciii_clinical.labevents` le
      ...
      AND le.charttime BETWEEN (ch.control_onset_time - interval '96' hour) AND ch.control_onset_time
      AND le.ITEMID in
    (
