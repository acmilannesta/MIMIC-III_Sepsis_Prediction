/*
Extract 48 hours of lab time series of sepsis case icustays before onset.
---------------------------------------------------------------------------------------------------------------------
...
CREATE OR REPLACE TABLE `bdhfa2021.project.case_48h_hourly_labs` AS
SELECT
  pvt.icustay_id
  , pvt.subject_id 
  , pvt.chart_time 
 ...
 --     WHEN itemid = 50889 and valuenum >  10000 THEN null -- mg/dl or mg/L! C-Reactive Protein  Blood Chemistry
  ...
  from `bdhfa2021.project.cases` ch -- was icustays ie (changed it below as well)
    ...
    LEFT JOIN `physionet-data.mimiciii_clinical.labevents` le
      ...
      AND le.charttime BETWEEN (ch.sepsis_onset - interval '48' hour) AND ch.sepsis_onset
      AND le.ITEMID in
    (
