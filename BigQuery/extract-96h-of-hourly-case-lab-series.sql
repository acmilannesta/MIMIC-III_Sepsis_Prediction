/*
Extract 96 hours of lab time series of sepsis case icustays before onset.
---------------------------------------------------------------------------------------------------------------------
- based on https://github.com/BorgwardtLab/mgp-tcn/blob/master/src/query/extract-55h-of-hourly-case-lab-series_ex1c.sql
---------------------------------------------------------------------------------------------------------------------
*/
...
CREATE OR REPLACE TABLE `bdhfa2021.project.case_96h_hourly_labs` AS
SELECT
  pvt.icustay_id
  , pvt.subject_id 
  , pvt.chart_time 
 ...
 --     WHEN itemid = 50889 and valuenum >  10000 THEN null -- mg/dl or mg/L! C-Reactive Protein  Blood Chemistry
  ...
  from `bdhfa2021.project.cases` ch -- was icustays ie (changed it below as well)
    ...
    LEFT JOIN `physionet-data.mimiciii_clinical.labevents` le
      ...
      AND le.charttime BETWEEN (ch.sepsis_onset - interval '96' hour) AND ch.sepsis_onset
      AND le.ITEMID in
    (
